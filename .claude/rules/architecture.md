...
- `voice_tell.py` — voice notes in the HQ topic: `parse_voice_instruction` deterministically extracts target + instruction (drop leading "tell/скажи", try first 1–3 words via `resolve_agent`, longest span first; no LLM), then presents a structured preview reusing the `/tell` confirmation callbacks. Delivery never happens without an explicit tap. Voice branch is entered from `voice_handler.handle_voice_message` (lazy import, gated on `is_hq_topic`); transcription itself is the shared `transcribe_voice_message` helper.
- `notifier.py` — background loop (started by `bootstrap.start_hq_notifier`, gated on `hq_topic_id` + `hq_notify_interval > 0`): diffs per-window notify states (`needs_you`/`done`/`dead`/`stale`; calm states never notify) and posts one message per transition into the HQ topic. First tick primes silently (restart-safe); dedup via module-level `_last_notified`; posts only for users whose HQ chat_id is recorded (`_record_hq_chat` in hq_commands / text / callback paths). Optional daily digest (`CCGRAM_HQ_DIGEST_TIME`, local HH:MM) rides the same loop and posts the `/brief` view once per day. Torn down in `shutdown_runtime` + `reset_for_testing`.
